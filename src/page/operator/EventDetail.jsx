...
function EventDetail(props) {
    ...
    useEffect(() => {
        setIsLoading(true);
        operatorApi.getEventDetailsById(eventId, getEventDetailSuccess, getEventDetailFail);
    }, []);
    ...
    const handleApproveEvent = () => {
        ...
        operatorApi.updateEventStatus(eventId, "APPROVED", null,
            () => {
                messageService.showMessage("Duyệt sự kiện thành công!", MESSAGE_TYPES.SUCCESS);
                setIsLoading(false);
            },
            (error) => {
                messageService.showMessage("Duyệt sự kiện thất bại!", MESSAGE_TYPES.ERROR);
                setIsLoading(false);
                navigate("/error", {
                    state: {
